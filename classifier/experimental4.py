...
from torch_geometric.nn import MessagePassing, global_mean_pool, global_max_pool, global_add_pool, GATv2Conv, BatchNorm, PANConv, GCNConv, Linear, ResGatedGraphConv, PDNConv, AntiSymmetricConv
...
# python classifier/experimental4.py --gpu_id 0 --dataset_name adni --seed 10 --n_folds 5 --epochs 100 --batch_size 4 --learning_rate 0.001 --hidden_dim 116 --dropout 0.7 --filter_size 6 --heads 1 --patience 10 --test_size 0.1 --pooling mean --percentile 0.9 --n_layers 1
...
class Net(torch.nn.Module):
    def __init__(self, functional_groups, num_nodes, hidden_dim, edge_dim, out_dim, heads=1, atlas=116, dropout=0.7, pooling='mean', filter_size=6, n_layers=1):
        ...
        for i in range(n_layers):
            self.layers.append(AlternateConvolution(in_features_v=hidden_dim, out_features_v=hidden_dim, in_features_e=edge_dim, out_features_e=edge_dim, node_layer=False))
            self.layers.append(ResGatedGraphConv(hidden_dim, hidden_dim, edge_dim=edge_dim))
            self.layers.append(LeakyReLU())
            self.layers.append(Dropout(p=dropout)) 
            self.layers.append(AntiSymmetricConv(hidden_dim))
            self.layers.append(BatchNorm(hidden_dim))
            self.layers.append(AlternateConvolution(in_features_v=hidden_dim, out_features_v=hidden_dim, in_features_e=edge_dim, out_features_e=edge_dim, node_layer=True))
            self.layers.append(ResGatedGraphConv(hidden_dim, hidden_dim, edge_dim=edge_dim))
            self.layers.append(LeakyReLU())
            self.layers.append(Dropout(p=dropout)) 
            self.layers.append(AntiSymmetricConv(hidden_dim))
            ...
            self.layers.append(AntiSymmetricConv(hidden_dim))
            self.layers.append(BatchNorm(hidden_dim))
        self.layers_bn = BatchNorm(hidden_dim)
        self.layers_dropout = Dropout(p=dropout)
        self.gat = GATv2Conv(hidden_dim, hidden_dim, heads=heads, dropout=dropout, edge_dim=edge_dim)
        self.lin = Linear(hidden_dim*heads, out_dim)
    ...
    def forward(self, data):
        ...
        # print_with_timestamp(f'(1) x shape: {x.size()}, {x.mean()}, {x.std()}')
        x = self.node_bn1(x)
        h = []
        n_adj = []
        ...
        x = self.node_bn2(x)
        x = self.activation(x)
        # print_with_timestamp(f'(2) x shape: {x.size()}, {x.mean()}, {x.std()}')
        edge_attr = data.edge_attr
        for i, layer in enumerate(self.layers):
            if i % 6 == 0:
                x, edge_attr = layer(x, edge_attr, data.edge_adj, n_adj, data.transition)
                ...
                x = layer(x)
            # print_with_timestamp(f'(2)({i}) x shape: {x.size()}, {x.mean()}, {x.std()}')
        x = self.layers_bn(x)
        x = self.layers_dropout(x)
        # print_with_timestamp(f'(3) x shape: {x.size()}, {x.mean()}, {x.std()}')
        x = self.gat(x, data.edge_index, edge_attr)
        #print_with_timestamp(f'x shape: {x.size()}')
        # print_with_timestamp(f'(4) x shape: {x.size()}, {x.mean()}, {x.std()}')
...
for fold, (train_index, test_index) in enumerate(kf.split(dataset, dataset.labels.to('cpu'))):
    ...
    model = Net(functional_groups, n_nodes, hidden_dim, edge_dim, out_dim, dropout=dropout, heads=heads, filter_size=filter_size, n_layers=n_layers).to(device)
