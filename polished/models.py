...
from torch_geometric.nn import global_mean_pool, global_add_pool, GCNConv, Linear, GINConv, GPSConv, ResGatedGraphConv, GINEConv
...
class BrainNetGCN(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        edge_index = data.edge_index
        for layer in self.layers:
            x = layer(x, edge_index, edge_weight=data.edge_attr)
            x = F.relu(x)
...
class BrainNetGPS(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, functional_groups=None, dropout=0.5, heads=4):
        ...
        self.lin1 = Linear(in_channels+self.encemb.embedding_dim, hidden_channels)
        for _ in range(num_layers):
            nn = Sequential(
...
class BrainNetResGated(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers, functional_groups=None, dropout=0.5):
        ...
        self.encemb = BrainContext(functional_groups)
        self.convs = torch.nn.ModuleList()
        self.n_rois = in_channels
        in_channels += self.encemb.embedding_dim
        for _ in range(num_layers):
            conv = ResGatedGraphConv(in_channels, hidden_channels, act=F.relu)
            self.convs.append(conv)
            in_channels = hidden_channels
        self.lin1 = Linear(hidden_channels, self.n_rois)
        self.lin2 = Linear(self.n_rois, out_channels)
        self.dropout = dropout
...
def get_model(args):
    ...
    elif model_name == 'resgated':
        return BrainNetResGated(in_channels=in_channels, hidden_channels=hidden_dim, out_channels=out_channels, num_layers=n_layers, dropout=dropout, functional_groups=groups)
    else:
